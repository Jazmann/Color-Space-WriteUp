...
-Chromophores, briefest of summaries from Optics of Skin, from summary further down. Include the incident light, transmission light, scattering, absorption, etc. The two processes.

Two processes in the skin: scattering and absorption. Luckily, the chemicals responsible for both are different. So, scattering chemicals != absorbing chemicals. So we only have to worry about absorption. The part of it that affects the color is the absorption chemicals, which are the chromophores. The scattering chemicals are important because they determine the average path length of the light through the skin. And, so the path length determines the amount of light which is absorbed; the longer the path length, the more light is absorbed for any given concentration of a chromophore.


-Say what the chromophores are, mention there are a variety, but will focus on the four main chromophores: oxy-hemo, deoxy, two kinds of melanin. At this point we can say "okay, so um, we know the absorption spectra for these chromophores, now what are we gonna choose as our term?" It's variously called "molar absorptivity, molar extinction coefficient, and molar absorption coefficient." All are <epsilon>. All are equivalent terms. Now, this is where we mention the Bear-Lambert law thing, so the actual absorbance (NOT absorption) depends on the path length of light through the skin and the concentration of the chemical, the chromophore. So, to apply the molar apsortivity, we need the average path length of skin and the average concentration of the chromophore and their beer-lambert law relation which is just the product of them all. Absorptibity in wiki. Annoyingly, the units are not consistent because chemistry, and we reference the page linked to from wikipedia, or in the notebook. But the SI units are m^2/mo. And so we, um, we mention where we got the data from (in the notebook), OMLC website where the absorption spectra for lotsa junk is. But they have absorption for, like, two forms of hemoglobin and two forms of melanin, eumelanin and bemelanin (or something to that effect). So, first task was to put this into consistent units and that's what this stuff is about.
