 ...
 slack(Cb_{min}) \le Cb \le  slack(Cb_{max})
  ...
  \begin{figure}[h!]
    \centering
      ...
      \caption{The Regions in the chromatic space. ~ Indicates values which will undergo redistribution.}  \label{fig:PartitioningRegions}
  \end{figure}
  ...
  Substituting into the distribution function gives us
  \begin{equation}
  -\frac{2^n \left(\text{erf}\left(\frac{2 \mu +1}{2 \sqrt{2} \sigma }\right)
  +\text{erf}\left( \frac{2^{\frac{1}{2}-2 n} \left( x-\mu  2^{2n-1}\right)}{\sigma }\right)\right)}
  {\text{erf}\left(\frac{2 \mu -1}{2 \sqrt{2} \sigma }\right)-\text{erf}\left(\frac{2 \mu +1}{2 \sqrt{2} \sigma }\right)}
  \end{equation}
