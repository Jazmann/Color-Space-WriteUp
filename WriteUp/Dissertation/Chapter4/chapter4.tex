...
\subsubsection{Force Analogue Shape Detection}\label{sec:ForceAnalogue}
...
Now we refine these points; because we know the orientation of the frame, we can determine which points correspond to the top edge of the digit, and which points correspond to the bottom. We now put them into neutral orientation as in steps a-c in Figure \ref{fig:ModelingFingertip}. The points can be easily classified as top edge, bottom edge and tip, just as before. 

We now find the distance of the corresponding points and model elements. For the straight edges, if we have $n$ points classified as lying along the top edge, then we divide the top line into $n$ points, each equally spaced along the line; the points on the image and the points on the model are now $1:1$ correspondence, and to find the difference between the two, we simply subtract. The same is done for the bottom edge. For the arc of the ellipse, we need to perform the comparison in the model's coordinates, i.e. where the center of the ellipse is at zero and the orientation of the ellipse is zero. In the model, we know that the tip of the trapezian lies on the ellipse, and from the sides, we know the location of these two points in the image points. The image points can then be rotated and translated so that these two points correspond in both the transformed image points into the model coordinates. We find the distance from the ellipse to the points using the equation <insert here>. The rotation and translation to the model coordinates is simply reversed, giving us vectors from the model to the corresponding points on the image. 

The position of the tip can now be found by taking the mean average of these vectors; subtracting the mean average from the vectors leaves a set which corresponds to the orientation. This is found by taking the inner product of the point position in the model with this rotational vector. For each point, this is analogous to a force acting at that point, producing a rotational moment about the center of the model, which is the center of the ellipse. Summing these moments gives us a rotational force on the model, and using the following equation <insert here>, the orientation can be found.
