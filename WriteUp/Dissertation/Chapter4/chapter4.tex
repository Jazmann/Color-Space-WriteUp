...
\subsubsection{Set the scale space.}\label{sec:EllipticalFitToTheTip}
...
The small scale space is made using the largest number of small factors as possible to achieve the desired image size. This gives the greatest flexability in scaling the mid-size.
\begin{table}
...
The medium scale space can be finding  $i_m$ and $j_m$ which minimise $i_m + \frac{\log(3)}{\log(2)} j_m$  and which satisfy  $2^{-i_m} 3^{-j_m} - \lceil \frac{w_m}{w} \rceil \ge 0$, $i_m\le i_s$ and $j_m \le j_s$
