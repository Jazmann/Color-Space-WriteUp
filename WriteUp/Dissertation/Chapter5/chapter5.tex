...
6. Blood flow patterns in the retina; good indication of eye health. Image the retina; could use algorithm given its sensitivities, we may be able to adapt it to discriminate between oxygenated and deoxygenated blood flow. We could use the image alignment so that, with a simple lens adaptor which fits over the phone's camera, one could photograph their own retina daily and measure the difference overtime. We could use the alignment algorithms to monitor any burst blood vessels, even in the eyeball itself; you could take a picture of the eye. 
