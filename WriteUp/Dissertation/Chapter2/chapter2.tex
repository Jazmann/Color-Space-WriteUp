...
\right)
\bigotimes
\fR[\theta] \right] \quad &\text{and} \quad \qS[n] = \left(
\begin{array}{c}
 1  \\
 2^{2-n } \\
 2^{2-n }  \\
...
Since every row in $\fR$ always has an element equal to 1, we will exploit the full range of the integer type. 
...
We are interested in the error introduced by the quantization, so we introduce the perturbation $\dfR[\theta]$ associated with the transform $\fR[\theta]$ produced by the rounding i 
