...
This allows us to relate the functions to each other, since they are now essentially the same function, and $\delta\theta$ has the same domain; the values they take have the same range, despite being in different positions in the matrix. 
...
Since every row in $\mathbf{fR}$ always has an element equal to 1, we will cover the full range of the integer type.
