...
% The matrix which is used in the algorithm \aR=\fSs \otimes \qR 
\newcommand\aR[1][]{
  \ifstrempty{#1}{
    \mathbf{aR}
  }{
    \mathbf{aR}\left( #1 \right)
  }
} 
...
\newcommand\nS[1][]{
  \ifstrempty{#1}{
    ...
    \vec{\mathbf{L}^{\um 1}}\left( #1 \right)
  }
} 
...
\newcommand{\tRange}{\text{tRange}}        % Rotated values after integer rotation range 
\newcommand{\aMax}{\text{aMax}}            % Rotated values after integer rotation integer range maximum
\newcommand{\aMin}{\text{aMin}}            % Rotated values after integer rotation integer range mimimum
\newcommand{\aRange}{\text{aRange}}        % Rotated values integer range
...
we can now rewrite the factored rotation as shown in \ref{table:factoredRotationMatrix}
\begin{table}[h]
\begin{tabular}{|c|c|c|}
\hline \rule[-2ex]{0pt}{5.5ex}  
$\Theta$ & 
...
% The Ordering function which puts dqE into the correct form for the other regions  \dqR[\theta]=\dqEO[ \dqRe[\theta] \theta] \otimes \dqs[\theta],
\newcommand\dqEO[1][]{
  \ifstrempty{#1}{
...
% The sign of the perturbation matrix \dqR[\theta]=\dqEO[ \dqRe[\theta] \theta] \otimes \dqs[\theta],
\newcommand\dqs[1][]{
  \ifstrempty{#1}{
    \mathbf{\delta qS}
  }{
...
% The absolute sign of the perturbation matrix by for row construction \dqR[\theta]=\dqEO[ \dqRe[\theta] \theta] \otimes \dqs[\theta],
\newcommand\dqas[1][]{
  \ifstrempty{#1}{
    \mathbf{\delta qAS}
  }{
...
\begin{enumerate}
\item \textbf{Find the working type and range.} 
...
\begin{gather*}
 ...
 \Sigma^+ &= \text{erf}\left(\frac{\mu }{\sqrt{2} \sigma }\right) 
  ...
  \end{array}
\end{gather*}
The length of the axis $\tRange$ after rescaling is given by \ref{eq:CombinedRotationRange} and the axis limits are easily found.
\begin{gather}
 \tRange(\theta,\mu,\sigma)   =  \min\left\{K \delta(\mu,\sigma) \right. ,  \left. \mathbf{L}(\theta) \right\}  \srcRange \\
 \begin{aligned}
  \tMin(\theta,\mu,\sigma)   & =
 \begin{pmatrix}
 0 \\
  \frac{-\tRange_2(\theta,\mu_2,\sigma_2) }{2} \\  
  \frac{-\tRange_3(\theta,\mu_3,\sigma_3) }{2} \\  
 \end{pmatrix} & 
  \tMax(\theta,\mu,\sigma)   & =
 \begin{pmatrix}
 ...
 \end{aligned}
\end{gather}
and the compression ratio $\kappa$ \ref{eq:newCompressionRatio}
\begin{equation}\label{eq:newCompressionRatio}
\kappa(\theta) = \max\left\{ \frac{1}{\delta(\mu,\sigma) }   , \frac{K}{\mathbf{L}(\theta)} \right\}   
\end{equation}
\\
\item \textbf{Set the distribution region boundary constants.} 
...
\begin{algorithm}[H]
 \KwData{$\Keep _2$ the starting point for the linear walk \;
$\tau_{p}$ the acceptable divergence from linearity }
 \KwResult{$ \{\eKeep_1, \eKeep_2\} $ the extended boundaries}
 $x \leftarrow \Keep_2$  \tcc*[r]{$dis(x)$ is the distribution function}
 \While{$dis(x) > x + \left\lfloor\text{dis}\left(\Keep _2\right)\right\rfloor  -\Keep _2 - \tau_{p}$}{
  x++ \;
 } 
 $\eKeep_2 \leftarrow x$ \;
 $\eKeep_1 \leftarrow \Keep _1 - (\eKeep_2 - \Keep _2)$ \tcc*[r]{Exploiting a symetry dis(x)}
 \KwRet{$\eKeep_1$ ,$\eKeep_2$ }
...
\\
\item \textbf{Suggest a new value for $\theta$.} 
...
%\end{gather*}
 ...
 \SetKwProg{Fn}{Function}{is}{end}
      \Fn{$\thetaX{i, \alpha, \beta, n}$}{
       \KwData{$i$ integer index of the intercept \quad
        $n$ the source bit depth }
       \KwResult{$\thetaX{i} $ the angular position of the intercept and the maximum perturbation at that value}
       $\meanIndx \leftarrow  2^{n-3}$  ; \quad 
        $\iota(i,n)  \leftarrow  \left\lfloor 7\ \meanIndx-\sqrt{i^2 - 2 i \; \meanIndx + 49\ \meanIndx^2 }\right\rfloor $ \;
        $\extremaTheta{b}{\mathbf{l} } = \arctan\left(\frac{\mathbf{l} \; 2^{2-n}}{\sqrt{3}}       \right)$ ; \quad 
        $\extremaTheta{a}{\mathbf{l} } = \arctan\left(\frac{\mathbf{l} \sqrt{3} }{2^n - \mathbf{l}}\right)$\;
        \eIf($i + \iota$ is even){$ i + \iota \bmod{2} = 0$}{
                 $\maximaTheta{a} \leftarrow \extremaTheta{a}{i+\iota+1}  $ ;\quad
                 $\minimaTheta{a}  \leftarrow \extremaTheta{a}{i+\iota} $ \;
                 $\maximaTheta{b} \leftarrow \extremaTheta{b}{i-\iota-1} $ ;\quad
                 $\minimaTheta{b}  \leftarrow \extremaTheta{b}{i-\iota}  $\;
          }{
                 $\maximaTheta{a}  \leftarrow \extremaTheta{a}{i+\iota} $ ;\quad
                 $\minimaTheta{a}  \leftarrow \extremaTheta{a}{i+\iota+1}  $ \;
                 $\maximaTheta{b} \leftarrow \extremaTheta{b}{i-\iota}  $ ;\quad
                 $\minimaTheta{b}  \leftarrow \extremaTheta{b} {i-\iota-1} $ \;
         }
        $\thetaX{i} \leftarrow \left\{\frac{
        \beta \minimaTheta{b} (\minimaTheta{a}  -\maximaTheta{a} )+\alpha  \minimaTheta{a} (\maximaTheta{b}-\minimaTheta{b})  }{
        \beta  (\minimaTheta{a} -\maximaTheta{a})+\alpha  (\maximaTheta{b}-\minimaTheta{b})
        },\frac{
        \alpha  \beta  (\minimaTheta{a}-\minimaTheta{b})  }{
...
To find the angular value closest to $\theta$ which produces a perturbation less than the tolerance $\tau$ the algorithm finds the closest value which satisfies the tolerance in the positive and negative directions and returns the nearest one to the requested value of $\theta$. the algorithm is guaranteed to find a value which satisfies the tolerance in each direction within a $\frac{\pi}{6}$ region because the perturbation is zero at the ends of the $\frac{\pi}{6}$ region.
 
\begin{algorithm}[H]
\SetKwProg{Fn}{Function}{is}{end}
 \KwData{$\alpha$, $\beta$ the relative importance of the channels \;
  \quad\quad\quad  $\theta$ the requested angle \;
   \quad\quad\quad $\tau$ the tolerance}
 \KwResult{$\vartheta$ the suggested value for $\theta$ and 
 $h$ the predicted maximum perturbation.}
 $\delta\theta  \leftarrow \theta \mod{\frac{\pi}{6} } $\;
 $\Theta  \leftarrow \theta - \delta\theta $\;
  $\meanIndx \leftarrow  2^{n-3}$\;
 $ i_{\circleddash} \leftarrow i_{\oplus} \leftarrow \frac{\meanIndx \tan (\delta \theta ) \left(\sqrt{3} \tan (\delta \theta )+7\right)}{\tan (\delta \theta )+\sqrt{3}} $ \;
 $\left\{ \vartheta_{\circleddash}, h_{\circleddash} \right\} \leftarrow \left\{ \vartheta_{\oplus}, h_{\oplus} \right\} \leftarrow \thetaX{i, \alpha, \beta, n}$ \;
 \While{$h_{\oplus} > \tau$}{
  $i_{\oplus}++$ \;
   $\left\{ \vartheta_{\oplus}, h_{\oplus}\right\} \leftarrow \thetaX{i_{\oplus}, \alpha, \beta, n}$
 } 
 \While{$h_{\circleddash} > \tau$}{
   $i_{\circleddash}--$ \;
    $\left\{ \vartheta_{\circleddash}, h_{\circleddash}\right\} \leftarrow \thetaX{i_{\circleddash}, \alpha, \beta, n}$
  } 
  \eIf{ $\vartheta_{\oplus} -\delta\theta < \delta\theta - \vartheta_{\circleddash}$ }{
  $\left\{ \vartheta, h\right\}  = \left\{\Theta + \vartheta_{\oplus}, h_{\oplus}\right\}  $ }{
  $\left\{ \vartheta, h\right\}  = \left\{\Theta + \vartheta_{\circleddash}, h_{\circleddash}\right\} $
  }
 \KwRet{$\left\{ \vartheta, h\right\} $ }
  \caption{Suggest a new value for $\theta$}
 \end{algorithm}
 

\item \textbf{Set the integer rotation matrix.} 
...
\end{cases} \\ 
 \fSs[\theta]  =
\begin{cases}
\left( \begin{smallmatrix}  1 \\    1 \\   1\\ \end{smallmatrix} \right) & 0                   \leq (\theta  \bmod \frac{2 \pi }{3} ) < \frac{  \pi }{6} \\
\left( \begin{smallmatrix}  1 \\  -1 \\   1\\ \end{smallmatrix} \right) & \frac{\pi }{6} \leq  (\theta  \bmod \frac{2 \pi }{3} ) < \frac{  \pi }{3} \\
\left( \begin{smallmatrix}  1 \\  -1 \\ -1\\ \end{smallmatrix} \right) & \frac{\pi }{3} \leq  (\theta  \bmod \frac{2 \pi }{3} ) < \frac{  \pi }{2} \\
\left( \begin{smallmatrix}  1 \\    1 \\ -1\\ \end{smallmatrix} \right) & \frac{\pi }{2} \leq  (\theta  \bmod \frac{2 \pi }{3} ) < \frac{2\pi }{3} 
\end{cases} 
\end{gather*}
...
\end{align*}
\\
\item \textbf{Design the distribution function.} 
...
%\begin{align*}
%\mathbf{Q}_{discard}     &\leftarrow \tau_{discard}     \le    1-\discard_2 + \discard_1 \\
%\mathbf{Q}_{distribute} &\leftarrow \tau_{distribute}  \le    \discard_2 - \ekeep_2 + \ekeep_1 - \discard_1 \\
%\mathbf{Q}_{keep}        &\leftarrow  \tau_{keep}         \le     \ekeep_2 - \ekeep_1 
%\end{align*}
...
%\begin{align*}
%\Discard_1^{q} &= \aRange \discard_1 +\aMin&
%\eKeep_1^{q} &= \aRange \ekeep_1 +\aMin \\
%\Discard_2^{q} &= \aRange \discard_2 + \aMin&
%\eKeep_2^{q} &= \aRange \ekeep_2 + \aMin&
%\end{align*}
%
% \begin{algorithm}[H]
% \SetKwProg{Fn}{Function}{is}{end}
%      \Fn{discard{t, n}}{
%       \KwData{$t$ the channel value  \quad
%        $n$ the source bit depth }
%       \KwResult{$d$ the redistributed channel value}
%       $\Discard_1^{q} \leftarrow \aRange \discard_1 + \aMin $; \quad 
%       $\Discard_2^{q} \leftarrow \aRange \discard_2 + \aMin $ \;
%       $\eKeep_1^{q}    \leftarrow \aRange \ekeep_1   + \aMin $; \quad 
%       $\eKeep_2^{q}    \leftarrow \aRange \ekeep_2   + \aMin $ \;
%       \SetAlgoLined
%       \uIf{$t < \Discard_1^{q}$}{
%       \KwRet{$\dstMin$ }
%       }
...
% \SetKwProg{Fn}{Function}{is}{end}
%      \Fn{chooseDistro}{
       \KwData{$\{\eKeep_1, \eKeep_2\}$ the extended boundaries of the information preserving region\;
       $ \{\discard_1, \discard_2\} $ the boundaries of the region which discards all information \;
        $0<\tau<1$ the tolerances for counting a region as significant. }
       \KwResult{$pDis$ a function which performs the re-distribution for a channel.}
       \begin{flalign*}[l]
       \mathbf{Q}_{discard}     & \leftarrow \tau_{discard}     \le    1-\discard_2 + \discard_1 \\
       \mathbf{Q}_{distribute} & \leftarrow \tau_{distribute}  \le    \discard_2 - \ekeep_2 + \ekeep_1 - \discard_1 \\
       \mathbf{Q}_{keep}         & \leftarrow \tau_{keep}         \le     \ekeep_2 - \ekeep_1 
        \end{flalign*}
        \begin{flalign*}
       \Discard_1^{q} &\leftarrow \aRange \discard_1 + \aMin &
       \Discard_2^{q} &\leftarrow \aRange \discard_2 + \aMin \\
       \eKeep_1^{q}    &\leftarrow \aRange \ekeep_1   + \aMin &
       \eKeep_2^{q}    &\leftarrow \aRange \ekeep_2   + \aMin 
       \end{flalign*}
          \SetAlgoLined
          \uIf{$\mathbf{Q}_{keep}  \wedge \mathbf{Q}_{discard} \wedge \mathbf{Q}_{distribute} $}{
          \KwRet{
          \begin{equation*}
          pDis(x) = \begin{cases}
          ...
          \text{dis}(\Discard_2) + \eKeep_2 - \eKeep_1 - \text{dis}(\eKeep_2) + \text{dis}(\eKeep_1) & x \ge \Discard_2^q \\
          \end{cases}
          \end{equation*} }
          }
          \uElseIf{$\mathbf{Q}_{keep}  \wedge \mathbf{Q}_{discard} \wedge \lnot \mathbf{Q}_{distribute} $}{
           \KwRet{
           \begin{equation*}
           pDis(x) = \begin{cases}
           \dstMin & x \le \Discard_1^q \\
           \Scale \; x  + \dstMin  & \Discard_1^q \le x \le \Discard_2^q  \\
           \Discard_2 + \dstMin  & x \ge \Discard_2^q \\
           \end{cases}
           \end{equation*} }
          }
          \Else($\mathbf{Q}_{keep}  \wedge \lnot \mathbf{Q}_{discard} \wedge \lnot \mathbf{Q}_{distribute} $){
            \KwRet{
            \begin{equation*}
            pDis(x) = \Scale \; x + \dstMin
            \end{equation*} }
            }
    %}
 \caption{An algorithm which returns an appropriate distribution function for each channel}
\end{algorithm}
