...
% \dfRe[\phi] &= 2^{2-n} \text{Round}\left[2^{n-2} \fRe[\phi ]\right]-\fRe[\phi ] \\
...
\begin{pmatrix}
 0                                     & 0                                      & 0                     \\
 \dqRe[ \thetaA ]              & 0                                      & -\dqRe[ \thetaA ] \\
 \dqRe[ \Pii{6} -\thetaA]  & - \dqRe[ \Pii{6} -\thetaA] & 0                     \\
\end{pmatrix}
  ,\theta \right]
\end{align*}
...
\begin{align*}
\nR[\theta] & = \nRa[\theta]+\dnR[\theta] \\
&= \nS[\theta] \otimes\rS \otimes \fSO \left[ \fSe[ \thetaA ] \right] \otimes  \qS[ n ]  \otimes \fRO \left[ \qRe[ \theta, n ]  + \dqRe[ \theta, n ]  \right] 
\end{align*}

defining

\begin{align*}
\Scale[\theta, n]   & =\nS[\theta] \otimes\rS \otimes \fSO \left[ \fSe[\thetaA] \right] \otimes  \qS[ n ]  \\
...
the perturbation to the rotated channel elements $\dW$ is found for an input set of pixel values $\rgb$
...
\right)
\bigotimes
\left( \qR[\theta] + \dqR\right)
\end{equation}
because $\fRO$ works equivalently for both $\qR$ and $\fR$
\begin{align}
\dfR[ \theta ]  &= \fRO \left[  \fRe[\theta] -\left(
\begin{array}{c}
 1  \\
 2^{2-n } \\
 2^{2-n }  \\
...
\begin{pmatrix}
 0 & 0 & 0 \\
 \dfRe[\phi]& 0 & \dfRe[-\phi] \\
 \dfRe[\frac{\pi }{6}-\phi]   & \dfRe[\phi-\frac{\pi }{6}] & 0 \\
\end{pmatrix}
 }
\end{equation}

The perturbation introduced to each channel is further simplified by recognizing that $\fRe[-\phi]=-1-\fRe[\phi]$, which allows us to find $\dfRe[-\phi]=-\dfRe[\phi]$ i.e. that the sum of the two perturbations equals zero. we can analyze the quantization error in terms of just one function $\dfRe$.
