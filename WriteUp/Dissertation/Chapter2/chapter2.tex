...
\begin{align}\label{eq:generalisedPerturbation}
...
\begin{algorithm}[H]
 \KwData{$x = \Keep _2$ the starting point for the linear walk}
 \KwResult{$\eKeep _2$ the extended boundary}
 $y = dis(x)$ \tcc*[r]{$dis(x)$ is the distribution function}
 \While{$dis(x) > \left\lfloor\text{dis}\left(\Keep _2\right)\right\rfloor + x -\Keep _2 - \tau_{p}$}{
  x++ \;
 } 
 $\eKeep_2=x$ \;
 $\eKeep_1=\Keep _1 - (\eKeep_2 - \Keep _2)$ \;
 \caption{Finding the extended region in which all the information is kept in C++}
\end{algorithm}


\\
\item \textbf{Suggest a new value for $\theta$.} stuff \\
