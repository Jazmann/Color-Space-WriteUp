...
\newcounter{algosection}[subsubsection]
\newcommand{\algosection}[1]{\stepcounter{algosection} \textbf{\thealgosection. #1}}
...
%\end{gather*}
 ...
 \begin{algorithmic}
      \Function {thetaX}{$ i, \alpha, \beta, n $}
       \Require{$i$ integer index of the intercept \quad
        $n$ the source bit depth }
       \Ensure{$\thetaX{i} $ the angular position of the intercept and the maximum perturbation at that value}
...
\begin{algorithm}[H]
 ...
 \begin{algorithmic}
  ...
  \Require{ $\theta$ the requested angle}
  \Require{ $\tau$ the tolerance}
 \Ensure{ $\vartheta$ the suggested value for $\theta$ and  $h$ the predicted maximum perturbation.}
\State  $\delta\theta  \gets \theta \mod{\frac{\pi}{6} } $\;
 \State $\Theta  \gets \theta - \delta\theta $\;
 \State  $\meanIndx \gets  2^{n-3}$\;
 \State $ i_{\circleddash} \gets i_{\oplus} \gets \frac{\meanIndx \tan (\delta \theta ) \left(\sqrt{3} \tan (\delta \theta )+7\right)}{\tan (\delta \theta )+\sqrt{3}} $ \;
\State  $\left\{ \vartheta_{\circleddash}, h_{\circleddash} \right\} \gets \left\{ \vartheta_{\oplus}, h_{\oplus} \right\} \gets \thetaX{i, \alpha, \beta, n}$ \;
 \While{$h_{\oplus} > \tau$}
 \State  $i_{\oplus}++$ \;
...
%       $\Discard_1^{q} \gets \aRange \discard_1 + \aMin $; \quad 
%       $\Discard_2^{q} \gets \aRange \discard_2 + \aMin $ \;
%       $\eKeep_1^{q}    \gets \aRange \ekeep_1   + \aMin $; \quad 
...
%\end{algorithm}
 ...
 \begin{algorithm}[H]
  ...
  \begin{algorithmic}
       ...
       \State \begin{tabular}{llrl}
       $\mathbf{Q}_{discard} $    & \multicolumn{3}{l}{ $\gets \tau_{discard}     \le    1-\discard_2 + \discard_1$} \\
       $\mathbf{Q}_{distribute}$ & \multicolumn{3}{l}{$\gets \tau_{distribute}  \le    \discard_2 - \ekeep_2 + \ekeep_1 - \discard_1$} \\
       $\mathbf{Q}_{keep} $        & \multicolumn{3}{l}{$\gets \tau_{keep}         \le     \ekeep_2 - \ekeep_1$}\\
       $\Discard_1^{q}$ &$\gets \aRange \; \discard_1 + \aMin$; &
       $\Discard_2^{q}$ &$\gets \aRange \; \discard_2 + \aMin$ \\
       $\eKeep_1^{q}$    &$\gets \aRange \; \ekeep_1   + \aMin$; &
       $\eKeep_2^{q}$    &$\gets \aRange \; \ekeep_2   + \aMin$ 
       \end{tabular} \;
         ...
         \State   \begin{equation*}
           ...
           \end{equation*} 
            ...
            Error; The tolerances or something else must be wrong!
            \EndIf
            \textbf{Return} {$pDis(x)$}
\end{algorithmic}
\end{algorithm}
