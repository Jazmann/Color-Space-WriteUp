...
\begin{align*}
a_{2m+1} & =     \hat{\phi}_{a}(m)     -      \hat{\phi}_{b}(m)  &
a_{2m} &=     \| \hat{\phi}_{b}(m-1) -     \hat{\phi}_{a}(m) \|  \\
b_{2m+1} &= \| \check{\phi}_{b}(m)     - \check{\phi}_{a} (m+1) \| &
b_{2m} &= \check{\phi}_{a}(m)     - \check{\phi}_{b}(m) \\
...
To construct a distribution function, we first need to describe the relationship of the error function to the Gaussian fit, and then produce a function with the appropriate range and domain. For a Gaussian fit with an amplitude A, a mean of $\mu$, and a standard deviation of $\sigma$, where $\mu$ and t lie in a source range $\tRange$ from $\tMin$ to $\tMax$. The cumulative distribution is found by integrating from $\tMin$ to the point t, as can be seen in (\ref{eq:ErfDefinition}):

\begin{equation}\label{eq:ErfDefinition}
  \int _{\tMin}^{\text{t}} A \frac{e^{-\frac{(x-\mu )^2}{2 \sigma ^2}}}{\sqrt{2 \pi } \sigma }dx=\frac{1}{2} A \left(\text{erf}\left(\frac{\text{x}-\mu }{\sqrt{2} \sigma }\right)-\text{erf}\left(\frac{\tMin-\mu }{\sqrt{2} \sigma }\right)\right)
\end{equation}
