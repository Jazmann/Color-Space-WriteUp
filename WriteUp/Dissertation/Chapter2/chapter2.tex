...
\text{recognising that the rounded part can only take certain values} \\
...
\begin{align}
 ...
 \check{\phi}_{b} = & \arctan\left(\frac{m 2^{3-n}}{\sqrt{3}}       \right) \quad & \text{with}\quad   m =  0,1,2,3 \cdots2^{n-3} \\
 \hat{\phi    }_{a}  = & \arctan\left(\frac{\sqrt{3} (2 m+1)}{-2 m+2^n-1}  \right) &
 \hat{\phi   }_{b}  = & \arctan\left(\frac{(2 m+1) 2^{2-n}}{\sqrt{3}}          \right) \quad&\text{with}\quad   m =  0,1,2,3 \cdots2^{n-3}-1 \\
 \check{\phi}_{a} (m)&< \check{\phi}_{b}(m)  <  \check{\phi}_{a} (m+1)& \hat{\phi}_{a}(m)   &< \hat{\phi}_{b}(m)  <\hat{\phi}_{a}(m+1) 
\end{align}
...
\begin{align*}
a_1 & = \hat{\phi    }_{b}(m) - \hat{    \phi}_{a}(m) & a_2 & = \hat{\phi}_{a}(m+1) - \hat{\phi}_{b}(m)  \\
b_1 & = \check{\phi}_{b}(m) - \check{\phi}_{a}(m) & b_2 & = \check{\phi}_{a}(m+1) - \check{\phi}_{b}(m)  \\
\tau_1 &\ge \frac{\check{\phi}_{b}(m)     - \check{\phi}_{a}(m)}{\hat{\phi}_{b}(m)     - \hat{\phi}_{a}(m) +\check{\phi}_{b}(m)     - \check{\phi}_{a}(m)} &
\tau_2 &\ge \frac{\check{\phi}_{a}(m+1) - \check{\phi}_{b}(m)}{\hat{\phi}_{a}(m+1) - \hat{\phi}_{b}(m)+\check{\phi}_{a}(m+1) - \check{\phi}_{b}(m)}\\
\end{align*}
